...
Run if needed `npx playwright install chrome`

### 3. Custom weather app

- Show tool that it can implicitly call with `#get_forecast`
- Show prompt that it can be called with `/run_cloths_recommendation`
- Show resource and how it can be used in the prompt, and implicitly call it with `#get_cloths` if needed

### 4. Build app

Build an app with Astro and TailwindCSS.
...
- Comet prompt injectying with Reddit comment
...
- TypeSpec or Postman MCP generator.
